  ...
  inputFocus: false,
	...
	setFileManagerSize: function(curCont, doParse) {
		...
		if(typeof fmContSettings[curCont].fmHeight == 'string' && fmContSettings[curCont].fmHeight.indexOf('%') != -1) {
			parentHeight = fmCont.parentNode.style.height ? fmCont.parentNode.offsetHeight : fmTools.getWindowHeight() - 80;
			height = Math.round(parentHeight * parseInt(fmContSettings[curCont].fmHeight) / 100);
			doResize = true;
		}
	...
	viewMenu: function(id, curCont) {
		...
		switch(id) {
			...
			default:
				if ((id >= 0) && (
					(fmEntries[curCont][id].name=='course.xml') ||
					(fmEntries[curCont][id].name=='exam.xml') ||
					(fmEntries[curCont][id].name=='preexam.xml') ||
					(fmEntries[curCont][id].name=='adminsettings.xml')
